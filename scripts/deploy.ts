...
task('ampl:deploy', 'Deploy helpcoin contracts').setAction(
  async (args, bre) => {
    console.log(args)
    ...
    // deploy HelpCoin
    const HelpCoin = await (
      await bre.upgrades.deployProxy(
        (await bre.ethers.getContractFactory('HelpCoin')).connect(deployer),
        [owner],
        {
    ...
    console.log('HelpCoin deployed to:', HelpCoin.address)

    // deploy Policy
    const HelpCoinPolicy = await (
      await bre.upgrades.deployProxy(
        (await bre.ethers.getContractFactory('HelpCoinPolicy')).connect(
          deployer,
        ),
        [owner, HelpCoin.address, BASE_CPI.toString()],
        {
          initializer: 'initialize(address,address,uint256)',
        },
      )
    ).deployed()
    console.log('HelpCoinPolicy deployed to:', HelpCoinPolicy.address)
    ...
    const orchestrator = await (
      ...
      .connect(deployer)
      .deploy(HelpCoinPolicy.address)
    console.log('Orchestrator deployed to:', orchestrator.address)
  },
)

task('ampl:upgrade', 'Upgrade helpcoin contracts')
  ...
  .setAction(async (args, bre) => {
    ...
    const supported = ['HelpCoin', 'HelpCoinPolicy']
